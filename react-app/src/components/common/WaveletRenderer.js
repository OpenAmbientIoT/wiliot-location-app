...
const WaveletRenderer = ({zones, width, height}) => {
    ...
    useEffect(() => {
        const selectedAssetRef = firestore.collection('wavelet-dev').doc('selectedAsset');
        ...
        const unsubscribeFromSelectedAsset = selectedAssetRef.onSnapshot(doc => {
            ...
            assetIds.forEach(assetId => {
                const unsubscribe = firestore.collection('wavelet-dev')
                    .doc('selectedAsset')
                    .collection(assetId)
                        ...
                        if (!snapshot.empty) {
                            ...
                            snapshot.docs.slice(0, BATCH_SIZE).forEach(doc => {
                                ...
                                if (processedUUIDs.current.has(data.uuid)) {
                                    ...
                                    const packetRef = firestore.collection('wavelet-dev')
                                        .doc('selectedAsset')
                                        .collection(assetId)
                                        .doc(doc.id);
