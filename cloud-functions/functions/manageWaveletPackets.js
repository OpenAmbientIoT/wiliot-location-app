...
exports.manageWaveletPackets = functions.firestore
    .document('wavelet-dev/selectedAsset')
    .onUpdate(async (change, context) => {
        ...
        if (previousValue.assetIds.length > 0 && newValue.assetIds.length === 0) {
          ...
          const collections = await db.collection('wavelet-dev').doc('selectedAsset').listCollections();
          ...
          await collectionIds.forEach(async (collectionId) => {
            ...
            const packetsCollectionRef = db.collection('wavelet-dev').doc('selectedAsset').collection(collectionId);
        ...
        } else if (previousValue.assetIds.length === 0 && newValue.assetIds.length > 0) {
        ...
        // checking if array of assetIds subtracted one, and deleting those packet collection docs
          ...
          const removedAssetIds = previousValue.assetIds.filter(x => !newValue.assetIds.includes(x));
          ...
          await removedAssetIds.forEach(async (removedAssetId) => {
            ...
            // Delete the documents in the packets collection
            const packetsCollectionRef = db.collection('wavelet-dev').doc('selectedAsset').collection(removedAssetId);
            const snapshots = await packetsCollectionRef.get();
